            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-slate-900">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-lg font-medium">Quản lý dòng tiền</h3>
                        <a href="{{ route('cash-flows.create') }}" class="px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:bg-blue-700 active:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                            Ghi nhận biến động
                        </a>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-slate-200">
                            <thead class="bg-slate-50">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-slate-500 uppercase tracking-wider">Số tiền</th>
                            ...
                            <tbody class="bg-white divide-y divide-slate-200">
                                    ...
                                    <tr class="hover:bg-slate-50 transition-colors duration-200">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $flow->type == 'deposit' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                {{ $flow->type == 'deposit' ? 'Nạp tiền' : 'Rút tiền' }}
                                            </span>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            ...
                                            <form action="{{ route('cash-flows.destroy', $flow) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-4 text-center text-sm text-slate-500">Không tìm thấy biến động tiền nào.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
