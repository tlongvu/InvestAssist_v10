                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="stock_id" name="stock_id" class="border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" required>
                                    @foreach($stocks as $stock)
                                        <option value="{{ $stock->id }}" {{ (old('stock_id', $stockTransaction->stock_id) == $stock->id) ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select id="type" name="type" class="border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" required>
                                    <option value="buy" {{ (old('type', $stockTransaction->type) == 'buy') ? 'selected' : '' }}>Buy</option>
                                    <option value="sell" {{ (old('type', $stockTransaction->type) == 'sell') ? 'selected' : '' }}>Sell</option>
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="quantity" class="block mt-1 w-full" type="number" min="1" name="quantity" :value="old('quantity', $stockTransaction->quantity)" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="price" class="money-input block mt-1 w-full" type="text" name="price" :value="old('price', number_format($stockTransaction->price, 0, '.', ','))" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="transaction_date" class="datepicker block mt-1 w-full" type="text" name="transaction_date" :value="old('transaction_date', $stockTransaction->transaction_date->format('Y-m-d'))" required />
