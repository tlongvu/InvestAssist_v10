...
The SessionStart hook (`gsd-check-update.js`) always writes to `./.opencode/cache/` via `os.homedir()` regardless of install type, so both paths must be cleared to prevent stale update indicators.
</step>
